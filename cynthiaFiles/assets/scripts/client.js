...
function mediamobilescreen() {
	return window.matchMedia("(orientation: portrait),(max-width: 800px)")
		.matches;
}
...
if (document.getElementById("cynthiapageinfoshowdummyelem") != null) {
	...
	if (typeof pagemetainfo.dates !== "undefined") {
		if (
			...
			dates = `<li>Posted: <span class="unparsedtimestamp">${new Date(
				pagemetainfo.dates.published * 1000,
			).toLocaleString()}</span></li>`;
		} else {
			dates = `
    <li>Posted: <span class="unparsedtimestamp">${new Date(
			pagemetainfo.dates.published * 1000,
		).toLocaleString()}</span></li>
    <li>Edited: <span class="unparsedtimestamp">${new Date(
